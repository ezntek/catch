from rlapi.rlapicfg import LINUX_SO_PATH, MACOS_DYLIB_PATH, WINDOWS_DLL_PATH
import sys
import re
import os
import platform
...
_lib_fname = {
    'win32': WINDOWS_DLL_PATH,
    'linux': LINUX_SO_PATH,
    'darwin': MACOS_DYLIB_PATH
}
...
_lib_default = os.path.join('/usr/lib/', _bitness, _lib_fname[_lib_platform])
...
@contextmanager
def scissor_mode(x: 'int', y: 'int', width: 'int', height: 'int') -> 'None':# type: (int, int, int, int) -> None
...
@contextmanager
def blend_mode(mode: 'int') -> 'None':# type: (int) -> None
...
@contextmanager
def mode2d(camera: 'Camera2D') -> 'None':# type: (Camera2D) -> None
...
@contextmanager
def mode3d(camera: 'Camera3D') -> 'None':# type: (Camera3D) -> None
...
@contextmanager
def shader_mode(shader: 'Shader') -> 'None':# type: (Shader) -> None
...
@contextmanager
def texture_mode(target: 'RenderTexture2D') -> 'None':# type: (RenderTexture2D) -> None
...
@contextmanager
def vr_stereo_mode(config: 'VrStereoConfig') -> 'None':# type: (VrStereoConfig) -> None
